...
export const INPUT_SCHEMAS: Record<string, FormSchema> = {
  '1040': {
    ...
    fields: [
      ...
      // Every manual field names its source document and box — an SOP-04
      // tester spent days reverse-engineering "what does Cati want here"
      // from return-line names alone. Users have W-2s and 1099s, not line
      // numbers; the description is the only text the compute forms show.
      { name: 'wages', type: 'number', required: true, description: 'Total wages, salaries, tips (W-2 box 1; add all W-2s)', category: 'income', irs_line: '1z' },
      { name: 'taxable_interest', type: 'number', required: false, description: 'Taxable interest (Form 1099-INT box 1; bank interest statements)', category: 'income', irs_line: '2b' },
      { name: 'ordinary_dividends', type: 'number', required: false, description: 'Ordinary dividends (Form 1099-DIV box 1a)', category: 'income', irs_line: '3b' },
      { name: 'qualified_dividends', type: 'number', required: false, description: 'Qualified dividends (Form 1099-DIV box 1b; taxed at LTCG rates)', category: 'income', irs_line: '3a' },
      { name: 'ira_distributions', type: 'number', required: false, description: 'Taxable IRA distributions (Form 1099-R box 2a with IRA/SEP/SIMPLE box checked)', category: 'income', irs_line: '4b' },
      { name: 'pensions_annuities', type: 'number', required: false, description: 'Taxable pensions and annuities (Form 1099-R box 2a)', category: 'income', irs_line: '5b' },
      { name: 'social_security', type: 'number', required: false, description: 'Taxable Social Security benefits (taxable portion of Form SSA-1099 box 5)', category: 'income', irs_line: '6b' },
      { name: 'capital_gains', type: 'number', required: false, description: 'Capital gain or loss (broker Form 1099-B / Schedule D net total)', category: 'income', irs_line: '7' },
      ...
      { name: 'student_loan_interest', type: 'number', required: false, description: 'Student loan interest paid (Form 1098-E box 1)', category: 'adjustments', irs_line: 'Sch1 L21' },
      { name: 'educator_expenses', type: 'number', required: false, description: 'Educator expenses deduction (your receipts; K-12 educators, capped)', category: 'adjustments', irs_line: 'Sch1 L11' },
      { name: 'itemized_deductions', type: 'number', required: false, description: 'Total itemized deductions (Schedule A: mortgage interest from Form 1098 box 1, state/local taxes, charitable gifts). If 0 or omitted, standard deduction is used.', category: 'deductions' },
      ...
      { name: 'k1_ordinary_income', type: 'number', required: false, description: 'Ordinary business income (Schedule K-1 box 1, from S-Corp or partnership)', category: 'k1' },
      { name: 'k1_w2_wages', type: 'number', required: false, description: 'W-2 wages from the K-1 QBI statement (1120-S box 17 code V / 1065 box 20 code Z attachment; used for the QBI limit)', category: 'k1' },
      { name: 'k1_ubia', type: 'number', required: false, description: 'UBIA of qualified property, from the same K-1 QBI statement (Section 199A attachment)', category: 'k1' },
      { name: 'qbi_from_k1', type: 'number', required: false, description: 'Section 199A qualified business income from the K-1 QBI statement (often equals box 1)', category: 'k1' },
      ...
      { name: 'ltcg_portion', type: 'number', required: false, description: 'Long-term portion of capital_gains (broker 1099-B long-term section; taxed at preferential 0/15/20% rates). If 0 or omitted, all gains taxed at ordinary rates.', category: 'income', irs_line: '7' },
      { name: 'net_se_income', type: 'number', required: false, description: 'Net self-employment income (Schedule C: 1099-NEC/1099-K receipts minus business expenses). Triggers SE tax (15.3%), half-SE deduction, and possibly Additional Medicare Tax.', category: 'income' },
      ...
      { name: 'withholding', type: 'number', required: false, description: 'Federal income tax withheld (W-2 box 2 plus any 1099 box 4 withholding)', category: 'payments', irs_line: '25d' },
      { name: 'estimated_payments', type: 'number', required: false, description: 'Estimated tax payments you sent the IRS during the year (your 1040-ES records)', category: 'payments', irs_line: '26' },
    ],
  },
  ...
  '1120S': {
    ...
    fields: [
      ...
      { name: 'schedule_k_interest', type: 'number', required: false, description: 'Portfolio interest income (Schedule K line 4) — e.g. 1099-INT Box 1 plus §1276 accrued market discount recharacterized on gain lots', category: 'schedule_k', irs_line: 'K-4' },
      { name: 'schedule_k_dividends_ordinary', type: 'number', required: false, description: 'Ordinary dividends (Schedule K line 5a) — 1099-DIV Box 1a', category: 'schedule_k', irs_line: 'K-5a' },
      { name: 'schedule_k_dividends_qualified', type: 'number', required: false, description: 'Qualified dividends (Schedule K line 5b) — 1099-DIV Box 1b', category: 'schedule_k', irs_line: 'K-5b' },
      { name: 'schedule_k_royalties', type: 'number', required: false, description: 'Royalties (Schedule K line 6)', category: 'schedule_k', irs_line: 'K-6' },
      { name: 'schedule_k_st_cap_gain', type: 'number', required: false, description: 'Net short-term capital gain/(loss) (Schedule K line 7) — from Schedule D / 1099-B aggregated', category: 'schedule_k', irs_line: 'K-7' },
      { name: 'schedule_k_lt_cap_gain', type: 'number', required: false, description: 'Net long-term capital gain/(loss) (Schedule K line 8a)', category: 'schedule_k', irs_line: 'K-8a' },
